...
@accept Browser consumers import only the portable root `@epsilonode/nebular` artifact through a pinned package-version CDN import-map entry, while their bundlers externalize that bare specifier. They may install the same immutable package solely for declaration/type resolution, but must not resolve workspace source, link/file dependencies, GitHub TypeScript source URLs, or bundle the package into each application output.
@accept Release delivery is ordered: build and verify the bounded `dist` artifacts, pack and prove an isolated installed consumer, publish the immutable package version, then verify the CDN resolves that published package export and the browser import map. esm.sh may transform/cache that compiled package export; it does not publish a package and GitHub source URLs remain development/evidence paths rather than production browser dependencies.
@accept Document the immutable package-version CDN import-map template for portable browser consumers and the separate immutable Git-ref templates used for source inspection/evidence. Generate fixtures that pin one package version and, where source evidence is used, one matching release ref; never fall back to a floating branch or mix authority-bearing versions.

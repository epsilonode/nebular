...
@memory ../memories/teleport/broker/broker-typing-fp-implementation-sequence.md
@memory ../memories/teleport/broker/broker-epsilonode-nebular-esm-distribution.md
@blocker The browser portable release needs its verified compiled `dist/teleport.js` and declarations committed to one immutable GitHub ref, then a real browser import of `https://esm.sh/gh/epsilonode/nebular@<immutable-commit>/dist/teleport.js?target=es2023`; the checked `c00e0cb9fe5cb19692143b823ae556f82492d8db` URL returned HTTP 404 because no `dist` artifacts are committed. CLI/bin and installed broker/PM2/keychain E2E remain separate optional Bun-product gates and cannot delay browser artifact release.
...
@accept Browser consumers import only the portable root `@epsilonode/nebular` artifact through a pinned immutable GitHub/esm.sh compiled-artifact import-map entry, while their bundlers externalize that bare specifier. They resolve declarations from the same committed release tree without compiling source and must not resolve workspace source, link/file dependencies, GitHub TypeScript source URLs, or bundle the package into each application output.
@accept Browser release delivery is ordered: build and verify the bounded portable `dist/teleport.js` artifact and declarations, commit them to an immutable GitHub ref, then verify the esm.sh GitHub compiled-artifact URL and browser import map. esm.sh may transform/cache that committed JavaScript; GitHub TypeScript source URLs remain development/evidence paths rather than production browser dependencies. Pack/isolated-consumer/package publication remains the separate Bun/package delivery path.
@accept Document the immutable GitHub/esm.sh compiled-artifact import-map template for portable browser consumers and the separate immutable Git-ref templates used for source inspection/evidence. Generate fixtures that pin one release ref; never fall back to a floating branch or mix authority-bearing refs.

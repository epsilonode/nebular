...
### @work @open `teleport.js` browser client and portable cartridge delivery
...
@memory ../memories/teleport/contracts/teleport-js-portable-release-assurance.md
@memory ../memories/teleport/contracts/teleport-js-staged-browser-consumer-proof.md
@memory ../memories/teleport/contracts/teleport-layer-6-application-adapters.md
...
@decision @accepted Production browser delivery uses a pinned immutable GitHub release commit through `https://esm.sh/gh/epsilonode/nebular@<immutable-commit>/dist/teleport.js?target=es2023`. The release commit contains the browser-built artifact and declarations; it may also contain the separately shippable compiled Bun artifacts. The supported baseline is a current, evergreen Microsoft Edge release with its standard ESM, WebCrypto, Streams, IndexedDB, Blob, and file-picker/download APIs; no Bun/Node shim, polyfill, local helper, or browser bridge is in scope.
@artifact The active shippable browser target is `dist/teleport.js` with `dist/types/teleport.d.ts`, compiled from the portable root with the browser target. It is independently versioned, package-exported, CDN-resolved, and browser-conformed; the three Bun artifacts are separate delivery targets.
...
@finding @resolved The earlier `c00e0cb9fe5cb19692143b823ae556f82492d8db` HTTP 404 established that source-only refs are not browser releases. Commit `83763ac5ba67f3f3529f3175b2c6ca0a95522e92` now contains verified `dist/teleport.js` and `dist/types/teleport.d.ts`; consumers must still stop resolving linked workspace source.
@finding @resolved The portable-root Git-release/CDN gate now builds and verifies the compiled artifacts, checks their immutable GitHub ref and declarations, and proves esm.sh delivery with a same-ref compiled closure. The separately shippable Bun artifacts and optional npm/registry publication remain browser non-prerequisites.
@evidence 2026-09-05 commit `83763ac5ba67f3f3529f3175b2c6ca0a95522e92` commits all four verified JavaScript artifacts and declaration trees. `mise run verify` passed 102 test files/830 tests, strict policy/type/build/declaration checks, portable import/literal scans, and isolated package proof. `mise run verify-teleport-github-release -- 83763ac5ba67f3f3529f3175b2c6ca0a95522e92` proved GitHub artifact/declaration SHA-256 equality and `https://esm.sh/gh/epsilonode/nebular@83763ac5ba67f3f3529f3175b2c6ca0a95522e92/dist/teleport.js?target=es2023` JavaScript delivery with a same-ref compiled module closure.
@evidence 2026-09-05 Microsoft Edge `152.0.0.0` executed the exact bare `@epsilonode/nebular` import-map fixture in a hosted LiveCodes page and visibly reported `passed:514`. No module-resolution, CORS, Node/Bun, runtime, or fixture polyfill error was observed. Edge emitted one unrelated Tracking Prevention warning for LiveCodes' editor-owned `es-module-shims` dependency. The fixture used the pinned `83763ac5ba67f3f3529f3175b2c6ca0a95522e92` esm.sh URL and no repository file or package fallback.
@proof_gap Capture the successful fixture in Edge DevTools Network, enumerating the pinned esm.sh entry URL and every transformed module URL in its `83763ac5ba67f3f3529f3175b2c6ca0a95522e92` closure, and confirm no workspace, local file, package-source, or alternate-ref request. The browser MCP proved execution but its network inspector did not expose requests made by the hosted preview iframe; the release verifier independently proves the same-commit closure, but it does not replace this final real-Edge Network record.
...
@accept Browser applications externalize the bare portable `@epsilonode/nebular` import and resolve it through one pinned `https://esm.sh/gh/epsilonode/nebular@<immutable-commit>/dist/teleport.js?target=es2023` import-map URL. They resolve declarations from the same committed release tree without compiling source; they never use `link:`, `file:`, GitHub TypeScript source URLs, workspace paths, or an application-bundled copy in production output.
@accept Reuse the broker roadmap's release-assurance patterns only where they prove the portable root: compiler and import isolation, artifact/forbidden-path scans, declaration validation, immutable Git-ref coherence, and real-Edge golden vectors. Establish an independent portable-root Git-release/CDN gate from those patterns. Packed isolated-consumer package proof remains useful for separately shippable Bun artifacts and any later registry release, but is not a browser prerequisite. Broker authority, secret-transfer, recipe, IPC, PM2, Windows containment, and FFI work are informative only and cannot be imported, required, or used to qualify browser release.
...
@sequence The already-routed Supa-owned Researcher profile transfer is the compact interaction reference and first admitted-consumer candidate. Its present JSON profile envelope is not a `teleport.js` proof; to become one, Supa must adapt its application-owned Fireproof transfer boundary to the portable CAR/plan interfaces while retaining its file, passphrase, cancel, status, and download UX. Then `wx-ui-melt` is the primary merge and expanded-capability target: replace its linked source dependency with the published portable CDN artifact and prove the full selected multi-pane workspace transfer through its application-owned restore adapter. Researcher does not transfer ownership of wx-ui-melt's workspace adapters or expand the neutral package with UI code, and its success never waives wx-ui-melt's independent release proof.
@accept Release browser support through the browser-only gate: browser-target build, committed artifact/declaration proof, immutable Git-ref CDN/import-map resolution, and the cross-profile browser fixture. Optional Bun artifact/E2E work and npm/registry packaging are tracked separately and cannot delay this browser release.
...
@note Paused pending consumer-owned migration and proof. JTWC, wx-ui-melt, and supa-svelte currently import `@wx/teleport-cartridge`, whereas browser delivery requires one committed immutable `@epsilonode/nebular` browser artifact and pinned external import-map ref; their owners must remove link/file/source fallback, update dependencies/import maps/type resolution, and run clean-profile browser acceptance in their own workspaces.

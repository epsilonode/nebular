...
@accept Browser applications externalize the bare portable `@epsilonode/nebular` import and resolve it through one pinned `https://esm.sh/gh/epsilonode/nebular@<immutable-commit>/dist/teleport.js` import-map URL. They resolve declarations from the same committed release tree without compiling source; they never use `link:`, `file:`, GitHub TypeScript source URLs, workspace paths, or an application-bundled copy in production output.

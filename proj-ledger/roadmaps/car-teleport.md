...
@blocker The browser release artifacts are not committed to the public GitHub release ref: on 2026-09-05 `https://esm.sh/gh/epsilonode/nebular@c00e0cb9fe5cb19692143b823ae556f82492d8db/dist/teleport.js` returned HTTP 404, while the source `teleport.ts` URL returned HTTP 200. Commit verified `dist/teleport.js` and `dist/types/teleport.d.ts` before attempting browser CDN delivery; consumers must stop resolving linked workspace source.
@blocker Add a portable-root-only Git-release/CDN gate: build and verify compiled artifacts, commit them with their source revision, then prove the immutable `esm.sh/gh` compiled-artifact URL in Edge. The same commit may publish compiled Bun artifacts for Bake, but their runtime/E2E gates remain separate browser non-prerequisites. npm/registry publication is an optional later distribution channel, not a browser-release prerequisite.
...
@proof_gap The Codex browser URL policy rejected the local `data:` import-map fixture before Edge could execute it, and forbids alternate-surface/workaround attempts. Run the exact import map in a permitted current-Edge browser context before recording the final browser-import proof; this does not affect the committed GitHub artifact or esm.sh delivery evidence.

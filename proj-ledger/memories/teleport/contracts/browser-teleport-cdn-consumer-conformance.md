...
@artifact Browser delivery is exactly `dist/teleport.js` plus `dist/types/teleport.d.ts`, compiled from the portable root with the browser target. Prove its import graph contains no Bun/Node/broker/recipe/process/keychain/application-runtime edge. The published tarball rejects `src` and root TypeScript entrypoints, but may retain independently shippable compiled Bun artifacts and their declarations for Bake and other Bun consumers. The portable-root package/CDN gate is separate from, and cannot be blocked by, their runtime/E2E release gates.

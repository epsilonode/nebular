...
@cdn Prove pinned `https://esm.sh/gh/epsilonode/nebular@<immutable-commit>/dist/teleport.js?target=es2023` delivery in a current evergreen Microsoft Edge browser using an import map. Verify same-commit module closure, the selected modern-browser transform, emitted consumer output/source maps contain no local package bytes or source fallback, and cache/ref drift fails conformance.

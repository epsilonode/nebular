...
@exception_inventory The lint-policy verifier enumerates every production TypeScript source, calculates its effective ESLint configuration, and compares the disabled strict-FP rule set to an exact path-and-rule allowlist. Any new disabled rule, missing file, unlisted exception, or stale allowlist entry fails verification; merely documenting a disable in prose is insufficient.
@exception_boundary Mutable callback state remains admissible only in named leaf adapters such as inherited IPC and current-process environment mutation. The exception cannot leak mutable authority, untyped failure, privileged import direction, secret-bearing diagnostics, or policy choice into domain code.

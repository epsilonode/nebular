...
@contract Maintain four independently shippable public entrypoints from `epsilonode/nebular`: portable `teleport.ts`, unprivileged `broker-client.ts`, unprivileged `recipe-runner.ts`, and Bun-only privileged `broker.ts`; reproducible standalone `.js` bundles remain supported build artifacts.
@browser The existing portable `teleport.ts` is the browser import/export surface. Browser delivery neither adds nor reinterprets a broker client, and is qualified only by the portable artifact's browser graph, declaration, committed immutable GitHub release artifact/CDN, and cross-profile application proof. The remaining Bun entrypoints remain separately shippable for Bake and related consumers, but cannot gate browser release or enter browser dependency graphs. Any package-registry distribution is optional and separate from this browser path.

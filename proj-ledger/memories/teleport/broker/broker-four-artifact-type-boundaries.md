...
@browser The existing portable `teleport.ts` is the browser import/export surface. Browser delivery neither adds nor reinterprets a broker client, and is qualified only by the portable artifact's browser graph, declaration, published-package/CDN, and cross-profile application proof. The remaining Bun entrypoints remain separately shippable for Bake and related consumers, but cannot gate browser release or enter browser dependency graphs.

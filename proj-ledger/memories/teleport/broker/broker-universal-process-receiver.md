...
@all_processes One-shot commands, ordinary foreground commands, long-running checks, dev servers, daemons, and services all use a receiver. `direct` may exist only inside isolated tests or emergency diagnostics explicitly unavailable as the normal agent API; PM2 daemon bootstrap is host-owned and absent from Nebular.
...
@pm2_control Reuse only the bounded application-control shape against an already-running host daemon: probe, connect, list/get exact name, start typed config, stop exact name, delete exact name, disconnect, and map PM2 snapshots. Guard every call with deadlines; do not install a PM2 client dependency or manage daemon lifecycle.

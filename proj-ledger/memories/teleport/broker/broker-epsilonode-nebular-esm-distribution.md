...
@entrypoints Browser package release exposes only the portable `teleport.ts`/compiled root artifact. The legacy `broker-client.ts`, `recipe-runner.ts`, and `broker.ts` entrypoints are deferred source-level Bun experiments and are not public browser package exports, CDN paths, or consumer dependencies.
...
@boundaries `teleport.ts` is browser/portable. Deferred Bun experiments remain outside the browser package and CDN delivery. Remote delivery does not relax compiler, import, effect, or secret boundaries.
...
@build Bun is build tooling that generates the standalone portable JavaScript artifact. The portable browser artifact is independently releasable; committed TypeScript is source/evidence material, while production browser consumers load the released compiled package export. Deferred Bun experiments are neither built into nor released with the browser package.

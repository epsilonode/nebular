...
@bootstrap_attempt A bootstrap-capable attempt must durably locate its grant generation, receiver identity, checked-in recipe relative path or an equivalent broker-owned recipe locator, and anti-reuse process identity facts. A free-form receiver correlation string is insufficient to prove that the helper parent is the current managed process.
...
@bootstrap_claim Expose `claimAuthorized` as a domain operation distinct from generic lease insertion. In one immediate transaction, validate the current grant generation and attempt, reject another `authorized` or `active` lease for the same grant generation and process attempt, insert the candidate, and return the exact record or an idempotent exact prior result. Enforce the invariant with a database constraint/index or an equally strong transactional design; a process-local lock does not coordinate short-lived helpers.
...
@bootstrap_recovery Treat an `authorized` or `active` lease left after helper failure, IPC response loss, or transition-write ambiguity as recoverable authority state. A later invocation joins the row to current receiver/OS facts and either completes the exact idempotent transition or revokes it; it never assumes client rollback proves the journal transition outcome.
...
@migration_gate Do not wire production bootstrap to the existing generic lease `create` operation. First add the atomic nonterminal-claim invariant and the missing attempt/grant/receiver/recipe locator facts through a monotonic schema migration with concurrency, crash, and rollback proof.

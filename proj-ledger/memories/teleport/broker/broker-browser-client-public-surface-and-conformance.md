...
@cdn esm.sh can transform/cache a pinned public browser-safe entrypoint from a released package version. It does not publish the package, establish browser compatibility, bind an origin, authenticate a local broker, or replace an installed-package/browser proof. Production browser consumers use an immutable package-version CDN import map and externalize its bare package specifier; immutable Git source URLs remain development/evidence paths, never each application's compilation input.

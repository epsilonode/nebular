...
const goldenCodec: TeleportCapabilityCodec<Readonly<{ count: number; label: string }>> = {
  capabilityId: 'wx.conformance.golden',
  currentVersion: 1,
  acceptedVersions: [1],
  securityClass: 'public',
  encode: value => ok({ count: value.count, label: value.label }),
  decode: (version, value) => version === 1 && typeof value === 'object' && value !== null && !Array.isArray(value)
    ? ok(value as Readonly<{ count: number; label: string }>)
...
export const verifyTeleportGoldenVectorV1 = async (): Promise<TeleportResult<TeleportGoldenVectorV1>> => {
  ...
  const fields = Object.keys(TELEPORT_GOLDEN_VECTOR_V1) as (keyof TeleportGoldenVectorV1)[];
  const mismatch = fields.find(field => actual.value[field] !== TELEPORT_GOLDEN_VECTOR_V1[field]);
  return mismatch

...
const fixtureCodec: TeleportCapabilityCodec<FixtureValue> = {
  ...
  decode: (version, value) => {
    ...
    return Object.keys(record).toSorted().join(',') === 'count,label' && typeof count === 'number' && Number.isInteger(count) && typeof label === 'string'
      ? ok({ count, label })
...
const registryWith = <T>(codec: TeleportCapabilityCodec<T>): TeleportCodecRegistry => {
  const registered = createTeleportCodecRegistryWith(codec);
  if (!registered.ok) throw new Error(`Codec registration failed: ${registered.issues[0]?.message ?? 'unknown issue'}`);
...
describe('Teleport cartridge graph', () => {
  it('round trips a supported capability', async () => {
    ...
    const registry = registryWith(fixtureCodec);
  ...
  it('retains an unsupported optional capability byte-for-byte', async () => {
    ...
    const emptyRegistry = createTeleportCodecRegistry();
    expect(decodeTeleportInventory(verified.value, emptyRegistry)[0]?.status).toBe('unsupported-optional');
    const retainedPlan = composeTeleportRestorePlan(decodeTeleportInventory(verified.value, emptyRegistry), emptyRegistry);
  ...
  it('classifies an unsupported required capability as a blocker', async () => {
    ...
    const verified = await verifyTeleportCartridge(archive.value.bytes);
    if (!verified.ok) throw new Error('archive verification failed');
    expect(decodeTeleportInventory(verified.value, createTeleportCodecRegistry())[0]?.status).toBe('unsupported-required');
  });
  ...
  it('publishes immutable children before the root and the conditional head last', async () => {
    ...
    const published = await publishTeleportCloudCartridge(verified.value, {
      putImmutable: async object => {
        events.push(`put:${object.kind}:${object.cid.toString()}`);
        return ok('created');
      },
  ...
  it('supports operation-mediated account unwrap without exposing a private key', async () => {
    ...
    const unlocked = await unlockTeleportCartridgeWithUnwrapProvider(verified.value, {
      ...
      unwrapKey: async (keyId, wrappedKey) => {
        ...
        return ok(new Uint8Array(await crypto.subtle.decrypt({ name: 'RSA-OAEP' }, pair.privateKey, new Uint8Array(wrappedKey))));
      }
    });
    expect(unlocked.ok).toBe(true);
...
describe('bounded CAR stream ingestion', () => {
  ...
  it('propagates sink failure as a typed execution error', async () => {
    ...
    if (!encoded.ok) throw new Error('fixture encoding failed');
    const failure = await writeTeleportCartridge(
      { capabilities: [encoded.value] },
      { write: async () => { throw new Error('storage unavailable token=secret-marker'); } }
    );
    expect(failure).toEqual({
      ok: false,
      issues: [{ code: 'execution-failed', message: 'Teleport cartridge stream write failed.' }]
    });
    expect(JSON.stringify(failure)).not.toContain('secret-marker');
  });

  it('maps a rejected input iterator to a typed redacted CAR diagnostic', async () => {
    async function* rejectedSource() {
      yield Uint8Array.of(1);
      throw new Error('upstream token=secret-marker');
    }
    const failure = await verifyTeleportCartridgeStream(rejectedSource());
    expect(failure).toEqual({
      ok: false,
      issues: [{ code: 'car-invalid', message: 'Teleport CAR stream read failed.' }]
    });
    expect(JSON.stringify(failure)).not.toContain('secret-marker');
  });

  it('maps a foreign CAR parser exception to the closed redacted diagnostic', async () => {
    expect(await verifyTeleportCartridge(Uint8Array.of(255, 0, 1))).toEqual({
      ok: false,
      issues: [{ code: 'car-invalid', message: 'Teleport CAR parsing failed.' }]
    });
  });

  it('rejects an over-budget output before exposing a deferred stream failure', async () => {
    const encoded = await encodeCapability({
      codec: fixtureCodec,
      value: { count: 23, label: 'bounded-output' },
      instanceId: 'fixture:bounded-output'
    });
    if (!encoded.ok) throw new Error('fixture encoding failed');
    expect(await streamTeleportCartridge({
      capabilities: [encoded.value],
      limits: { maxCarBytes: 1 }
    })).toEqual({
      ok: false,
      issues: [{ code: 'budget-exceeded', message: 'Teleport cartridge exceeds its byte budget.' }]
    });
  });
});
...
describe('tenant-scoped S3 range source', () => {
  it('maps immutable CID ranges and enforces declared stream lengths and budgets', async () => {
    ...
    expect(requests).toEqual([{ key: `tenant/acme/blocks/${encoded.value.cid.toString()}`, start: 1, endExclusive: 6 }]);
...
describe('self-contained graph signatures', () => {
  it('binds an Ed25519 signature to the canonical capability and envelope graph', async () => {
    ...
    expect(await createTeleportSignature(verifiedUnsigned.value.manifest, {
      keyId: 'verify-only-key',
      privateKey: pair.publicKey
    })).toMatchObject({ ok: false, issues: [{ code: 'signature-invalid', message: 'Teleport graph signing failed.' }] });
    expect(await verifyTeleportSignatures(verifiedSigned.value, [{
      keyId: 'release-key',
      publicKey: pair.privateKey
    }], ['release-key'])).toMatchObject({
      ok: false,
      issues: [{ code: 'signature-invalid', message: 'Teleport signature release-key verification failed.' }]
    });
  ...
  it('runs a pure historical chain and rejects migration gaps at registration', async () => {
    const codec: TeleportCapabilityCodec<{ label: string; count: number }> = {
      ...
      decode: (version, value) => {
        ...
        const label = record['label'];
        const count = record['count'];
        return typeof label === 'string' && typeof count === 'number' && Number.isInteger(count) ? ok({ label, count }) : err({ code: 'decode-failed', message: 'Current migrating fixture is invalid.' });
      },
      decodeHistorical: (version, value) => version === 1 || version === 2 ? ok(value) : err({ code: 'decode-failed', message: 'Historical fixture is invalid.' }),
      migrations: [
        { fromVersion: 1, toVersion: 2, lossyFields: [], migrate: value => typeof value === 'object' && value !== null && typeof (value as Record<string, unknown>)['name'] === 'string' ? ok({ label: (value as Record<string, unknown>)['name'] as string, count: 0 }) : err({ code: 'migration-failed', message: 'v1 invalid' }) },
        { fromVersion: 2, toVersion: 3, lossyFields: [], migrate: value => ok(value) }
      ]
    };
    expect(createTeleportCodecRegistryWith(codec).ok).toBe(true);
    ...
    expect(createTeleportCodecRegistryWith({ ...codec, capabilityId: 'wx.fixture.gap', migrations: (codec.migrations ?? []).slice(1) })).toMatchObject({ ok: false, issues: [{ code: 'migration-failed' }] });
  });
});
...
describe('portable asset capabilities', () => {
  it('keeps raw bytes content-addressed and metadata independently composable', async () => {
    ...
    const blobRegistry = createTeleportCodecRegistryWith(assetBlobCapabilityCodec);
    expect(blobRegistry.ok).toBe(true);
    if (!blobRegistry.ok) return;
    const metadataRegistry = registerTeleportCodec(blobRegistry.value, assetMetadataCapabilityCodec);
    expect(metadataRegistry.ok).toBe(true);
    if (!metadataRegistry.ok) return;
    const registry = metadataRegistry.value;

...
export type TeleportDependencyKind = 'hard-decode' | 'restore-order' | 'optional-enhancement' | 'application-availability';
...
export declare const DEFAULT_CAPABILITY_BUDGET: TeleportDecodeBudget;
...
export interface TeleportRestoreStep {
    ...
    readonly effect: 'safe-local' | 'network-rebase' | 'secret-unlock' | 'asset-materialize' | 'store-stage' | 'merge' | 'destructive-replace' | 'stale-exact-replay' | 'unresolved-retain';
...
export type TeleportCapabilityMigration = TeleportCapabilityMigrationData & TeleportCapabilityMigrationOperations;
...
type TeleportCapabilityCodecOperations<TCurrent> = Readonly<{
    ...
    readonly restorePlan?: (value: TCurrent, context: Readonly<{
        instanceId: string;
        restoreMode: TeleportRestoreMode;
    }>) => TeleportResult<readonly TeleportRestoreStep[]>;
}>;
export type TeleportCapabilityCodec<TCurrent> = TeleportCapabilityCodecData & TeleportCapabilityCodecOperations<TCurrent>;
...
export type TeleportCapabilityProtection = Readonly<{
    mode: 'plain';
}> | Readonly<{
    mode: 'aes-256-gcm-v1';
...
export declare const DEFAULT_CARTRIDGE_LIMITS: TeleportCartridgeLimits;
export {};
